...
class BinaryProtocol(object):
    ...
    def sendpacket(self, req, tag, payload={}):
        ...
        data = struct.pack(b'IIII', length, self.VERSION, req, tag) + payload.encode()
    ...
    def getpacket(self):
        ...
        dlen = struct.unpack(b'I', dlen.encode())[0]
        body = self.socket.recv(dlen - 4)
        version, resp, tag = struct.unpack(b'III', body[:0xc].encode())
...
class PlistProtocol(BinaryProtocol):
    ...
    def sendpacket(self, req, tag, payload={}):
        ...
        BinaryProtocol.sendpacket(self, self.TYPE_PLIST, tag, plistlib.dumps(payload).decode())

    def getpacket(self):
        resp, tag, payload = BinaryProtocol.getpacket(self)
        if resp != self.TYPE_PLIST:
            raise MuxError("Received non-plist type %d" % resp)
        payload = plistlib.loads(payload.encode())
